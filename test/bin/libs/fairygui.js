    ...
    class GObject {
        ...
        setSize(wv, hv, ignorePivot = false) {
            if (this._rawWidth != wv || this._rawHeight != hv) {
                ...
                this.displayObject && this.displayObject.event(fgui.Events.SIZE_CHANGED);
            }
        }
        ensureSizeCorrect() {
        }
        ...
        handleXYChanged() {
            ...
            if (this._displayObject) this._displayObject.pos(xv + this._pivotOffsetX, yv + this._pivotOffsetY);
        }
        handleSizeChanged() {
            if (this._displayObject) this._displayObject.size(this._width, this._height);
        }
        handleScaleChanged() {
            if (this._displayObject) this._displayObject.scale(this._scaleX, this._scaleY, true);
        }
        handleGrayedChanged() {
    ...
    class GComponent extends fgui.GObject {
        ...
        addChildAt(child, index = 0) {
            ...
            if (index >= 0 && index <= this._children.length) {
                ...
                throw "fgui 2575 Invalid child index";
            }
        }
        ...
        getChildAt(index) {
            if (index >= 0 && index < this._children.length)
                ...
                throw "fgui 2629 Invalid child index";
        }
        getChild(name) {
            var cnt = this._children.length;
            for (var i = 0; i < cnt; ++i) {
    ...
    class GList extends fgui.GComponent {
        ...
        scrollToView(index, ani = false, setFirst = false) {
            if (this._virtual) {
                ...
                var ii = this._virtualItems[index];
                if (ii == null) {
                    console.error("ii==null", "index:", index, "_virtualItems:", this._virtualItems);
                    return;
                }
                var pos = 0;
                var i;
...
(function (fgui) {
    class GProgressBar extends fgui.GComponent {
        ...
        update(newValue) {
            if (this._barObjectH instanceof fgui.GObject) {
                this._barObjectH.visible = newValue > 0.1;//解决进度条花屏问题 clong 2021.7.9
                // if (newValue == 0) console.warn("this._barObjectH.visible=false newValue: " + newValue);
            }
    ...
    class UIPackage {
        ...
        static loadPackage(resKey, completeHandler) {
            ...
            var descCompleteHandler = Laya.Handler.create(this, function (asset) {
                ...
                if (!asset) {//clong 2021.7.26
                    throw new Error("------->loadPackage error url: " + url);
                }
        ...
        getItemAsset(item) {
            switch (item.type) {
                case fgui.PackageItemType.Image:
                    if (!item.decoded) {
                        ...
                        if (sprite != null) {
                            ...
                            if (atlasTexture == null) {//clong 2021.6.22
                                item.texture = null;
                            } else {
                ...
                case fgui.PackageItemType.Atlas:
                    if (!item.decoded) {
                        let texture = fgui.AssetProxy.inst.getRes(item.file);//clong 2021.6.22
                        if (texture) {
                            item.decoded = true;
...
(function (fgui) {
    class ByteBuffer extends Laya.Byte {
        ...
        readBuffer() {
            ...
            if (!this.buffer) {//clong 2021.7.26
                throw new Error("------->ByteBuffer this.buffer=null");
            }
